...
const CesiumMap = forwardRef<MapHandle, MapProps>(function CesiumMap(props, ref) {
  ...
  useEffect(() => {
    ...
    // desktop: render at the device pixel ratio so raster tiles/labels stay crisp.
    // mobile: that means 4–9× the fragment work per frame on a 2–3× DPR screen,
    // which makes panning crawl — so let the browser pick a lighter resolution and
    // cap the backing store at ~1.5× to keep dragging smooth.
    viewer.useBrowserRecommendedResolution = coarse;
    if (coarse) viewer.resolutionScale = Math.min(window.devicePixelRatio || 1, 1.5);
    ...
    viewer.scene.globe.maximumScreenSpaceError = coarse ? 2 : 1;
